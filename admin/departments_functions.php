...
function reality_filter_activity_by_department( $has_activities, $activities_template, $template_args ) {
	...
	global $reality, $activities_template, $bp, $post;
	
	$user_id = $bp->loggedin_user->id;
	$user = new WP_User( $user_id );
	$department = $user->roles;
	
 	/* if ( isset( $reality->current_dept && $bp->current_component == 'activity' ) ) {
 	
 		$template_args['in'] = implode( ',', $reality->current_dept_activity );
 	
 	} */
	
	if ( !in_array( 'administrator', $department ) && !in_array( 'game-master', $department ) && !isset($bp->displayed_user->id) && get_post_type( $post) != 'reality_deals' ) {
