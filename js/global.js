...
jq(document).ready( function() {
	...
	jq('input[name="new-photo"]').change( function() {
	
		if ( jq(this).val() != '' ) {
		
			jq(this).addClass('has-file');
			jq(this).parent().find('a.new-photo-button').addClass('has-file');
		
		} else {
			
			jq(this).removeClass('has-file');
			jq(this).parent().find('a.new-photo-button').removeClass('has-file');
	...
	/* New posts */
	jq("form#whats-new-form").submit( function() {
		console.log('submitting post!!');
		if ( jq('input[name="new-photo"]').val() == '' ) {
			var button = jq(this);
	...
	jq('form.ac-form').not('.visible').hide();

	/* Hide excess comments */
	if ( jq('.activity-comments').length && !jq('.activity-comments').hasClass('no-js') )
	...
	jq('div.activity').click( function(event) {
		...
		if ( target.attr('name') == 'ac_form_submit' ) {
			...
			var ajaxdata = {
				...
				'audience_award': jq('form#' + form.attr('id') + ' .reality-deal-audience-awards input:radio:checked').val(),
				'reality_deal_id':	jq('form#' + form.attr('id') + ' .reality-deal-audience-awards input[name="reality_deal_id"]').val()
			};
			...
			jq.post( ajaxurl, ajaxdata,
				function(response)
				{
					...
					if ( response[0] + response[1] == '-1' ) {
						...
						form.fadeOut( 200,
							function() {
								if ( 0 == form.parent().children('ul').length ) {
									if ( form.parent().hasClass('activity-comments') && !form.parent().hasClass('deals-single') )
								...
								if ( jq('body').hasClass('single-reality_deals') ) {
									form.parent().children('ul').prepend(response).hide().fadeIn( 200 );
								} else {
						...
						if ( jq('form#' + form.attr('id') + ' .reality-deal-audience-awards input:radio:checked').length != 0 ) {
							jq( 'div.reality-deal-audience-awards' ).html('<div class="message">Thank you for voting!</div>');
						}
